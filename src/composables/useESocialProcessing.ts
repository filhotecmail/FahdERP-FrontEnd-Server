...
export function useESocialProcessing(mockData: ProcessingItem[]) {
  ...
  const simulateProcessing = async (itemIds: string[], operation: string) => {
    ...
    for (const itemId of itemIds) {
      ...
      if (itemIndex !== -1) {
        ...
        (mockData[itemIndex] as any).processingResult = outcome
      }
    }
  ...
  const processSelectedItems = async () => {
    ...
    if (result.isConfirmed) {
      await simulateProcessing(selectedItems.value, 'processing')
    }
  }
  ...
  const retifySelectedItems = async () => {
    ...
    if (result.isConfirmed) {
      await simulateProcessing(selectedItems.value, 'retifying')
    }
  }
  ...
  const deleteSelectedItems = async () => {
    ...
    if (result.isConfirmed) {
      await simulateProcessing(selectedItems.value, 'deleting')
    }
  }
  ...
  const getRowProcessingClass = (itemId: string) => {
    ...
    if (item && (item as any).processingResult) {
      const result = (item as any).processingResult
      switch (result) {
        case 'success':
          return 'row-success'
        case 'critics':
